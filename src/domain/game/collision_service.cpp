...
bool CollisionService::HasCollided(const BirdState &bird,
...
bool CollisionService::HasHitPipe(const BirdState &bird,
  ...
  /* Птица как круг */
  const float bird_left = bird.x - bird.radius;
  const float bird_right = bird.x + bird.radius;
  const float bird_top = bird.y - bird.radius;
  const float bird_bottom = bird.y + bird.radius;

  const float pipe_left = pipe.x; // да, у меня pipe как прямоугольник
  const float pipe_right = pipe.x + pipe.width;

  const bool overlaps_x = bird_right > pipe_left && bird_left < pipe_right;
